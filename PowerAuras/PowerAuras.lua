...
old_hasMainHandEnchant = nil
old_mainHandName = ""
old_mainHandExpiration = 0
old_mainHandCharges = 0
old_hasOffHandEnchant = nil
old_offHandName = ""
old_offHandExpiration = 0
old_offHandCharges = 0
...
function PowaCompareWeaponBuff(xnum)
	local hand
	local unit =  'player'
	
	for pword in string.gfind(PowaSet[xnum].buffname, "[^/]+") do
	
		if (strfind(strlower(pword), 'main')) then
			pword = string.gsub(pword,'main','')
			hand = 1
		elseif (strfind(strlower(pword), 'off')) then
			pword = string.gsub(pword,'off','')
			hand = 2
		else
			hand = 0
		end
		...
		if ( hand < 2 ) then
			...
			Powa_Tooltip:SetInventoryItem( unit, 16);
			for i=1, 23 do
			...
			Powa_Tooltip:Hide();
		elseif ( hand ==0 or hand ==2 ) then
			Powa_Tooltip:SetOwner(UIParent, "ANCHOR_NONE");
			Powa_Tooltip:SetInventoryItem( unit, 17);
			for i=1, 23 do
				local text = getglobal("Powa_TooltipTextLeft"..i):GetText();
				if ( not text ) then
					break;
...
function Powa_NewCheckBuffs() -- compare chaque nom de buff d'activation avec l'ensemble des buffs memorises
	...
	elseif (PowaSet[j].buffname == "" or PowaSet[j].buffname == " ") then -- ne fait rien si vide
		PowaSet[j].buffname = "";
	elseif (PowaCompareBuffDebuff(j) or PowaCompareWeaponBuff(j)) then -- buff actif
...
function Powa_UpdateTimer(numi)
	...
	elseif (PowaSet[numi].isdebuff or PowaSet[numi].isdebufftype) then
		...
		if (PowaModTest) then
			...
			if (Powa_Frames[numi].buffindex > 15) then
				local my, me, mc, oy, oe, oc = GetWeaponEnchantInfo();
			if (Powa_Frames[numi].buffindex == 16) then
					newvalue = me;
				else
					newvalue = oe;
				end
...
function Powa_GetTempBuffName(id)
	Powa_Tooltip:SetOwner(UIParent, "ANCHOR_NONE");
	Powa_Tooltip:SetInventoryItem("player", id)
	for i=1,23 do
		local toolTipText = getglobal("Powa_TooltipTextLeft" .. i):GetText()
		if not toolTipText then break; end
		local _, _, buffname = string.find(toolTipText, "^([^%(]+) %(%d+ [^%)]+%)")
		--PowaDebug( toolTipText)
		if buffname then
			--PowaDebug( buffname)
			return buffname
		end
...
function Powa_WeaponEnchantChanged()
	local hasMainHandEnchant, mainHandExpiration, mainHandCharges, hasOffHandEnchant, offHandExpiration, offHandCharges = GetWeaponEnchantInfo()
	local mainHandName = ""
	if hasMainHandEnchant then
		mainHandName = Powa_GetTempBuffName(GetInventorySlotInfo("MainHandSlot"))
	end
	local offHandName = ""
	if hasOffHandEnchant then
		offHandName = Powa_GetTempBuffName(GetInventorySlotInfo("SecondaryHandSlot"))
	end
	...
	if hasMainHandEnchant ~= old_hasMainHandEnchant or mainHandName ~= old_mainHandName or mainHandExpiration > old_mainHandExpiration or mainHandCharges ~= old_mainHandCharges or hasOffHandEnchant ~= old_hasOffHandEnchant or offHandName ~= old_offHandName or offHandExpiration > old_offHandExpiration or offHandCharges ~= old_offHandCharges then
		Powa_NewCheckBuffs();
	end
